...
37	1069	166	1037	172	258	1071	90	497	1219	145	1206	143	153	1067	510";
        ...
        {            
            ...
            CONTENT = new Int32[lines.Length][];
            for (int i = 0; i < lines.Length; ++i)
            {                
                String[] cells = lines[i].Split("\t");
                CONTENT[i] = new Int32[cells.Length];
                for (int j = 0; j < cells.Length; ++j)
                {
                    CONTENT[i][j] = Int32.Parse(cells[j]);
                }
            }            
        }

        static int EvaluateRowChecksum(int index)
        {
            int min = Int32.MaxValue;
            int max = Int32.MinValue;
            for (int i = 0; i < CONTENT[index].Length; ++i)
            {
                min = Math.Min(min, CONTENT[index][i]);
                max = Math.Max(max, CONTENT[index][i]);
            }
            Console.WriteLine("Checksum for row {0} id {1}", index, max - min);
            return max - min;
        }

        static int EvaluateRowEven(int index)
        {
            for (int i = 0; i < CONTENT[index].Length; ++i)
            {
                for (int j = 0; j < CONTENT[index].Length; ++j)
                {
                    if ((i != j) && (CONTENT[index][i] % CONTENT[index][j] == 0 && (CONTENT[index][i] >= CONTENT[index][j])))
                    {
                        Console.WriteLine("Checksum for row {0} id {1}", index, CONTENT[index][i] / CONTENT[index][j]);
                        return CONTENT[index][i] / CONTENT[index][j];
                    }
                }
            }
            Console.WriteLine("Checksum for row {0} id {1}", index, 0);
            return 0;
        }

        static int EvaluateChecksum()
        {
            int checksum = 0;
            for (int i = 0; i < CONTENT.Length; ++i)
            {
                checksum += EvaluateRowEven(i);
            }
            return checksum;
        }
        ...
        {
            ParseRaw();
            Console.WriteLine("Checksum = {0}", EvaluateChecksum());
        }
    }
}
